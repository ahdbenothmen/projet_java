...
const RECENT_DEMANDES = [
  ...
  { nom: "Personne 3",     niveau: "Licence 3", status: "refuse" },
...
const STATUS_STYLE: Record<string, { bg: string; color: string; label: string }> = {
  ...
  refuse:  { bg: "#FCEBEB", color: "#791F1F", label: "Non retenu" },
};
...
export default function DashboardAdmin() {
  ...
  return (
    <div id="dashboard-page" style={{ display: "flex", minHeight: "100vh", fontFamily: "'Segoe UI', sans-serif", background: "#F1EFE8" }}>

      {/* ── Sidebar ── */}
      <aside id="sidebar" style={{ width: "220px", background: "#fff", borderRight: "1.5px solid #D3D1C7", display: "flex", flexDirection: "column", padding: "24px 16px" }}>
        {/* Logo */}
        <div id="sidebar-logo" style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "32px" }}>
          <div style={{ width: "36px", height: "36px", background: "#185FA5", borderRadius: "10px", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
              <path d="M12 2a5 5 0 1 1 0 10A5 5 0 0 1 12 2zm0 12c5.33 0 8 2.67 8 4v2H4v-2c0-1.33 2.67-4 8-4z" fill="#fff" />
            </svg>
          </div>
          <div>
            <div style={{ fontSize: "13px", fontWeight: 700, color: "#2C2C2A" }}>UniAdmin</div>
            <div style={{ fontSize: "11px", color: "#888780" }}>Administration</div>
          </div>
        </div>
          ...
          {NAV_ITEMS.map((item) => (
            <div
              ...
              style={{
                ...
                color: activeNav === item ? "#185FA5" : "#2C2C2A",
                background: activeNav === item ? "#E6F1FB" : "transparent",
                cursor: "pointer",
              }}
            >
              {item}
        ...
        {/* Admin info */}
        <div id="sidebar-admin" style={{ borderTop: "1.5px solid #D3D1C7", paddingTop: "16px", display: "flex", alignItems: "center", gap: "10px" }}>
          <div style={{ width: "34px", height: "34px", borderRadius: "50%", background: "#185FA5", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: "12px", fontWeight: 700 }}>
            AD
          </div>
          <div>
            <div style={{ fontSize: "13px", fontWeight: 600, color: "#2C2C2A" }}>Admin</div>
            <div style={{ fontSize: "11px", color: "#888780" }}>admin@univ.tn</div>
          </div>
        </div>
      </aside>

      {/* ── Main ── */}
      <main id="dashboard-main" style={{ flex: 1, padding: "28px 28px", overflowY: "auto" }}>

        {/* Header */}
        <div id="dashboard-header" style={{ marginBottom: "24px" }}>
          <h1 style={{ fontSize: "20px", fontWeight: 700, color: "#2C2C2A", margin: 0 }}>Tableau de bord</h1>
          <p style={{ fontSize: "13px", color: "#888780", marginTop: "4px" }}>Vue générale — Année 2025/2026</p>
        </div>
        ...
        <div id="stats-statut" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "14px" }}>
          {[
            ...
            { id: "stat-refuse",  label: "Non retenus",   count: 8,  total: totalDemandes, color: "#A32D2D", bg: "#FCEBEB", bar: "#A32D2D" },
          ].map((s) => (
